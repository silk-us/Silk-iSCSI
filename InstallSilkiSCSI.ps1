...
if (Get-Module silkiscsi) {
    Remove-Module silkiscsi
}

$fullpath = $installDirectory + '/silkiscsi/' 

$moduleInfo = Test-ModuleManifest -Path ./silkiscsi/silkiscsi.psd1
...
Copy-Item ./SilkiSCSI/* -Destination $fullfolder -Recurse
Get-ChildItem $fullfolder -Recurse | Unblock-File

Import-Module silkiscsi

Get-Module silkiscsi